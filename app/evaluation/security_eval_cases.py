...
SECURITY_EVAL_CASES = (
    ...
    SecurityEvalCase("SEC-002", "T01", "graph_level_prompt_injection_enforcement",
                     "graph_runtime", SecurityExpectation.CONTROLLED, False,
                     "Dataset V2 Graph treats prompt text as untrusted input: server-trusted AccessContext supplies allowed metrics and scopes, and the governed executor receives a compiled contract rather than prompt-provided raw SQL."),
    ...
    SecurityEvalCase("SEC-008", "T06", "final_sql_region_predicate_validation",
                     "final_sql_enforcement", SecurityExpectation.CONTROLLED, False,
                     "Final PostgreSQL AST enforcement must preserve every governed Row Scope predicate as an actual top-level AND conjunct; semantic weakening such as scope_predicate OR TRUE is denied."),
    ...
    SecurityEvalCase("SEC-011", "T08", "repaired_sql_actual_predicate_preservation",
                     "repair_runtime", SecurityExpectation.CONTROLLED, False,
                     "Any Repaired SQL Candidate must remain linked to the original Governed Envelope and Compiled Contract and re-pass parameter, output, resource, Scope Predicate, and PostgreSQL AST enforcement before it can become executable evidence; automatic V2 Repair Runtime remains disabled."),
