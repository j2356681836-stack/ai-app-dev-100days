...
10. 不要使用中文字段别名。
11. 字段别名必须优先使用指标技术名。百分比类指标才乘以 100，并使用 ROUND(..., 2)，字段别名必须使用 指标技术名 + "_pct"。例如 refund_rate 输出 refund_rate_pct，channel_refund_rate 输出 channel_refund_rate_pct。ROI / 投资回报率不是百分比，不要乘以 100，字段别名必须使用 roi。
